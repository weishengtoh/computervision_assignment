...
class ImageEncoder(object):
    ...
    def __init__(self,
        ...
        self.input_var = tf.get_default_graph().get_tensor_by_name(layers[0] +
                                                                   ':0')
        self.output_var = tf.get_default_graph().get_tensor_by_name(layers[-1] +
                                                                    ':0')
...
def generate_detections(encoder, mot_dir, output_dir, detection_dir=None):
    ...
    try:
        ...
        if exception.errno == errno.EEXIST and os.path.isdir(output_dir):
            ...
            raise ValueError("Failed to created output directory '%s'" %
                             output_dir)
